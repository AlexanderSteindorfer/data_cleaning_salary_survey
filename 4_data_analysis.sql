-- Short analysis of the cleaned data.
...
-- Overview
-- Looking through the outcome I can already say that there is no clear
-- correlation between salary_annual and additional_compensation.
SELECT
    salary_annual,
...
ORDER BY additional_compensation_percent DESC;
...
-- This makes sense, as the additional compensation is highly dependent on the amount of sales
-- generated by an employee in sales.
-- Note: the last column I am selecting here will hold the factor showing how much higher
-- the average additional_compensation of a given industry is compared to the overall average.
-- The average additional_compensation in sales is therefore 2.28 times higher than the average
...
SELECT
    ...
    ROUND(AVG(additional_compensation / salary_annual *100)) AS avg_additional_compensation_percent,
    ROUND(AVG(additional_compensation / salary_annual *100) / 12, 2) AS factor_vs_overall
FROM salary_survey_us
    WHERE additional_compensation IS NOT NULL
...
-- experience in their field, so they are somewhat underrepresented.
SELECT 
    experience_in_field,
    COUNT(*) AS 'count'
FROM salary_survey_us
...
-- Correlation: salary_annual vs. experience_in_field,
-- independently of industry/sector
-- This again confirms the general correlation and the tendency of the two highest
-- experience classes to have lower payment.
SELECT 
    experience_in_field,
    ROUND(AVG(salary_annual))
FROM salary_survey_us
...
-- The conclusion is that the above 68 percent are not a proper guide, especially
-- given the fact that the ranking the query above creates shows that 20 percent
-- lead to a higher salary_annual than 100 percent on average, which is unrealistic.
SELECT
    industry,
    experience_proportion,
    ROUND(AVG(salary_annual))
